...
if ( ! class_exists( 'WWPDFSystemCheck' ) ) {

	class WWPDFSystemCheck {
		...
		public function __construct() {
			$this->options = get_site_option('waterwoopdf_systemcheck');
		}
		...
		private function dismissMessages() {
			if (isset($_REQUEST['dismissMessage']) && isset($_REQUEST['_wpnonce'])) {
				...
				if (wp_verify_nonce($nonce, "waterwoo-dismiss-{$key}")) {
					$this->options[$key] = false;
					update_site_option('waterwoopdf_systemcheck', $this->options);
				}
			}
		}
		...
		private function checkWordPressVersion() {
			...
			if (!function_exists('wp_enqueue_media')) {
				$error = "The WaterWooPDF plugin requires WordPress 3.7 or above. Please upgrade your WordPress installation.";
  ...
  /**
    ...
    private function checkRoleScoper() {
		...
		private function checkWpFooter() {
			...
			$error = "Uh oh! Required call to wp_footer() not found in file <b>{$file_path}</b>. <br /><br />Please check the <a href='http://codex.wordpress.org/Function_Reference/wp_footer'>wp_footer()</a> documentation and make sure your theme has a call to wp_footer() just above the closing </body> tag.";
		...
		private function printMessage($message, $key) {
			$nonce = wp_create_nonce( "waterwoo-dismiss-{$key}" );
			echo "<div id='message' class='updated'><p><b>Warning:</b> {$message}<br /><br /><a class='button' href='?page=waterwoo&dismissMessage={$key}&_wpnonce={$nonce}'>Hide</a></p></div>";
		}
	}
}
